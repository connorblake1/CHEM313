...
# def Langevin_step_constantF(x, V, beta, gamma, F, step_size): # CB: ignore, not used
#     x = torch.clone(x).detach()
#     x.requires_grad = True
#     gradient = torch.autograd.grad(outputs=V(x),
#                                     inputs=x, grad_outputs = torch.ones_like(V(x)),
...
# def Langevin_step_F(x, V, beta, gamma, F, step_size): # CB: not used
#     x = torch.clone(x).detach()
#     x.requires_grad = True
#     gradient = torch.autograd.grad(outputs=V(x),
#                                     inputs=x, grad_outputs = torch.ones_like(V(x)),
...
# def Underdamped_step(x, v, V, beta, gamma, step_size): # CB: not used
#     x = torch.clone(x).detach()
#     x.requires_grad = True
#     gradient = torch.autograd.grad(outputs=V(x),
#                                     inputs=x, grad_outputs = torch.ones_like(V(x)),
...
# def Langevin_step_net(x, V, beta, gamma, net, step_size, b = False): # CB: not used
#     x = torch.clone(x).detach()
#     #print(x)
#     x.requires_grad = True
#     gradient = torch.autograd.grad(outputs=V(x),
...
# def Underdamped_step_net(x, v, V, net, beta, gamma, k, us, step_size, single_sample = False, i = None): # CB: not used
#     x = torch.clone(x).detach()
#     x.requires_grad = True
#     if single_sample:
...
# def take_reporter_steps_stride(input_xs, V, beta, gamma, step_size, n_trajectories, n_steps, a_center, b_center, cutoff, adaptive = False): # CB not used
...
# def take_reporter_steps_constantF(input_xs, V, beta, gamma, step_size, F, n_trajectories, n_steps, a_center, b_center, cutoff, adaptive = False): # CB: not used
#     xs = torch.repeat_interleave(input_xs, n_trajectories, axis = 0).to(device)
...
#         if step_sizes.nonzero().size() == torch.Size([0,1]):
#             break
#     return xs.reshape([-1, n_trajectories, xs.size()[-1]]), (steps/step_size).cpu().detach().numpy(), running_weights/torch.sum(running_weights)#torch.ones(n_trajectories)/n_trajectories

# def take_reporter_steps_F(input_xs, V, beta, gamma, step_size, F, n_trajectories, n_steps, a_center, b_center, cutoff, adaptive = False): # CB: also not used
#     xs = torch.repeat_interleave(input_xs, n_trajectories, axis = 0).to(device)
#     running_weights = torch.ones(len(xs))
#     step_sizes = torch.repeat_interleave(step_size, xs.size()[0], axis = 0)
#     steps = torch.clone(step_sizes)
...
#         if step_sizes.nonzero().size() == torch.Size([0,1]):
#             break
#     return xs.reshape([-1, n_trajectories, xs.size()[-1]]), (steps/step_size).cpu().detach().numpy(), running_weights/torch.sum(running_weights)

# def take_reporter_steps_net(input_xs, V, beta, gamma, net, step_size, n_trajectories, n_steps, a_center, b_center, cutoff, adaptive = False, b = False): # CB: not used
#     xs = torch.repeat_interleave(input_xs, n_trajectories, axis = 0).to(device)
#     running_weights = torch.ones(len(xs))
...
#         if step_sizes.nonzero().size() == torch.Size([0,1]):
#             break
#     return xs.reshape([-1, n_trajectories, xs.size()[-1]]), (steps/step_size).cpu().detach().numpy(), running_weights/torch.sum(running_weights), torch.mean(torch.log(running_weights))

# def take_reporter_steps_underdamped(input_xs, input_vs, V, beta, gamma, step_size, n_trajectories, n_steps, a_center, b_center, cutoff, lag, adaptive = False): # CB: not used
#     xs = torch.repeat_interleave(input_xs, n_trajectories, axis = 0).to(device)
...
#             print('WOOO')
#             break
#     return xs.reshape([-1, n_trajectories, xs.size()[-1]]), vs.reshape([-1, n_trajectories, xs.size()[-1]]), (steps/step_size).cpu().detach().numpy()
            
# def take_reporter_steps_MFPT(input_xs, V, beta, gamma, step_size, n_trajectories, n_steps, a_center, b_center, cutoff, adaptive = False): # CB: not used
#     xs = torch.repeat_interleave(input_xs, n_trajectories, axis = 0).to(device)
#     step_sizes = torch.repeat_interleave(step_size, xs.size()[0], axis = 0)
...
# def calculate_reweighted_committor_estimates(xs, net, a_center, b_center, cutoff, n_trajectories, weights):  # CB: not used
#     zeros = torch.zeros(xs.size()[0]).to(device)
#     ones = torch.ones(xs.size()[0]).to(device)
#     a_estimates = torch.sigmoid(net(xs))
#     xs_for_var = torch.reshape(xs, [int(xs.size()[0]/n_trajectories), n_trajectories, 2])
...
# def calculate_committor_estimates_underdamped(xs, vs, net, a_center, b_center, cutoff, n_trajectories): # CB: not used
#     zeros = torch.zeros(xs.size()[0]).to(device)
#     ones = torch.ones(xs.size()[0]).to(device)
...
# def calculate_MFPT_estimates(xs, times, net, a_center, b_center, cutoff, n_trajectories): # CB: not used
#     ones = (torch.ones(xs.size()[0])).to(device)
#     zeros = torch.zeros(xs.size()[0]).to(device)
#     a_estimates = (net(xs))
...
# def calculate_committor_estimates_2(xs, net, a_center, b_center, cutoff, n_trajectories): # CB: not used
#     zeros = torch.zeros(xs.size()[0]).to(device)
#     ones = torch.ones(xs.size()[0]).to(device)
#     a_estimates = torch.sigmoid(net(xs))
#     a_estimates = torch.where(dist(xs, a_center) < cutoff, zeros, a_estimates)
...
# def calculate_committor_estimates_iter(xs, net, iter_net, a_center, b_center, cutoff, n_trajectories): # CB: not used
#     zeros = torch.zeros(xs.size()[0]).to(device)
#     ones = torch.ones(xs.size()[0]).to(device)
