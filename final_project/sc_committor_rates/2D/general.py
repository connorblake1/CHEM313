...
print(net(grid_input.reshape(-1,dim)),net(grid_input.reshape(-1,dim)).shape)
    ...
    for j in range(1): # Can go through multiple optimization steps per data collection step, if needed
        ...
        for m in range(100):
            ...
            for i in range(0, len(running_xs), batch_size):
                ...
                loss, individual_losses =  1*training.half_loss(net, running_xs.to(device)[indices], a_short_targets.to(device)[indices], b_short_targets.to(device)[indices], i_a, i_b, cmask)
                log_loss, individual_log_loss = 1*training.half_log_loss(net, running_xs.to(device)[indices], a_short_targets.to(device)[indices], b_short_targets.to(device)[indices], None, None, i_a, i_b, cmask)
