...
true_committor = np.transpose((np.load('MB_committor.npy'))) # TODO:CB where is this from?
...
for trial in range(1): # Can run multiple trials, if you'd like
    ...
    for step in range(n_opt_steps):
        ...
        if step == 0 or a_transit == True:
            ...
            a_reporter_energies = V(a_running_short_reporters.reshape([-1,2])[last_a_transit:])  # noqa: F821
            a_weights = torch.sigmoid(net(a_running_short_reporters.squeeze().reshape([-1, 2])))[last_a_transit:].detach() # noqa: F821
            a_weights = torch.where(dist(a_running_short_reporters.squeeze().reshape([-1, 2])[last_a_transit:], a_center) < cutoff, 0, a_weights) # noqa: F821
            _, a_indices = torch.sort(a_weights, descending = True)
            a_indices = a_indices[:1]
            a_xs = a_running_short_reporters.squeeze().reshape([-1, 2])[last_a_transit:][a_indices]# noqa: F821
        ...
        if step == 0 or b_transit == True:
            ...
            b_reporter_energies = V(b_running_short_reporters.reshape([-1,2])[last_b_transit:])# noqa: F821
            b_weights = torch.sigmoid(-net(b_running_short_reporters.squeeze().reshape([-1, 2])))[ last_b_transit:].detach()# noqa: F821
            b_weights = torch.where(dist(b_running_short_reporters.squeeze().reshape([-1, 2])[ last_b_transit:], b_center) < cutoff, 0, b_weights)# noqa: F821
            _, b_indices = torch.sort(b_weights, descending = True)
            b_indices = b_indices[:1]
            b_xs = b_running_short_reporters.squeeze().reshape([-1, 2])[last_b_transit:][b_indices]# noqa: F821
        ...
        if step % 10 == 9:
            ...
            axs['c'].text(-1.65, -0.25, "(c)", weight = 'bold', size = 12, zorder = 200)
            ...
            axs['d'].text(-1.65, -0.25, r"(d)", weight = 'bold', size = 12, zorder = 200)
            ...
            axs['a'].scatter(torch.reshape(running_xs, [-1, 2]).detach().numpy()[:,0], torch.reshape(running_xs, [-1, 2]).cpu().detach().numpy()[:,1], color = 'red', alpha = 1)
            axs['a'].scatter(torch.reshape(a_running_xs, [-1, 2]).cpu().detach().numpy()[-1,0], torch.reshape(a_running_xs, [-1, 2]).detach().numpy()[-1,1], color = 'blue', alpha = 1)
            axs['a'].scatter(torch.reshape(b_running_xs, [-1, 2]).cpu().detach().numpy()[-1,0], torch.reshape(b_running_xs, [-1, 2]).cpu().detach().numpy()[-1,1], color = 'green', alpha = 1)
            ...
            axs['a'].text(-1.65, -0.30, "(a)", weight = 'bold', size = 20, zorder = 200)
            ...
            axs['b'].legend([r'Rate Estimate A to B', r'Rate Estimate B to A', r'Analytical Rate A to B', r'Analytical Rate B to A'], prop={'size': 12})
            #axs['b'].text(-20, 1.3e-7, r"\text{\textbf{(b)}}", weight = 'bold', size = 20, zorder = 200)
            plt.tight_layout()
            fig.savefig("MB.pdf")
            plt.close()
